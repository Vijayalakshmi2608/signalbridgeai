...
import { MadeWithMagnusBadge } from "./MadeWithMagnus";
...
export function SignalBridgeShell({ children }: { children: ReactNode }) {
  ...
  const [offline, setOffline] = React.useState(() => typeof window !== "undefined" && window.localStorage.getItem("signalbridge:connectivity") === "offline");
  React.useEffect(() => { const handleConnectivity = (event: Event) => setOffline((event as CustomEvent<{ offline?: boolean }>).detail?.offline === true); window.addEventListener("signalbridge:connectivity", handleConnectivity); return () => window.removeEventListener("signalbridge:connectivity", handleConnectivity); }, []);
  ...
  return (
    <div className="min-h-screen bg-[#f4f7f9] text-[#152536]">
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-[246px] flex-col border-r border-[#d8e1e8] bg-[#0d1b2a] px-4 py-5 text-white transition-transform lg:translate-x-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <Link href="/offline" className={`rounded-xl border p-3 transition hover:bg-[#173448] ${offline ? "border-[#9e5252] bg-[#3b2329]" : "border-[#284b5b] bg-[#122b3b]"}`}><div className="flex items-center gap-2 text-xs font-semibold text-[#d5e4e8]">{offline ? <WifiOff size={14} className="text-[#ff9a92]" /> : <Wifi size={14} className="text-[#7ee2de]" />} {offline ? "OFFLINE EMERGENCY MODE" : "Connection stable"}</div><div className="mt-1.5 text-[11px] leading-4 text-[#86a1ad]">{offline ? "Using last synchronized information" : "Last sync 06:32 IST · Data is simulated"}</div></Link>
...
export function SignalMap({ compact = false, selectedPoint, onSelectPoint }: { compact?: boolean; selectedPoint?: GeoPoint; onSelectPoint?: (point: GeoPoint) => void }) {
  ...
  return <div className={`relative overflow-hidden rounded-xl border border-[#cbd9df] bg-[#dfeaec] ${compact ? "h-[230px]" : "h-[480px]"}`}><div ref={mapRef} role="img" aria-label="GeoShield map showing simulated flood warning zones, roads, shelters, and selected location" className="h-full w-full cursor-crosshair" /><div className="pointer-events-none absolute left-3 top-3 rounded-lg border border-white/70 bg-white/95 px-3 py-2 shadow-sm"><div className="font-mono text-[9px] font-semibold uppercase tracking-[.14em] text-[#27445b]">OpenStreetMap · GeoShield</div><div className="mt-1 text-[11px] text-[#71818b]">Click anywhere to assess this location</div></div></div>;
}
